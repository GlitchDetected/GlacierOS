...
}  __attribute__ ((__packed__)) PTINFO, *PPTINFO;
...
}  __attribute__ ((__packed__)) BPB, *PBPB;
...
}  __attribute__ ((__packed__)) EBPB, *PEBPB;
...
typedef struct _tagEBPB32 {
	...
	uint8_t label[11];			// volume label
	uint8_t system[8];			// filesystem ID
}  __attribute__ ((__packed__)) EBPB32, *PEBPB32;
...
typedef struct _tagLBR {
	...
	uint8_t sig_55;				// 0x55 signature byte
	uint8_t sig_aa;				// 0xaa signature byte
}  __attribute__ ((__packed__)) LBR, *PLBR;
